...
class Booking(db.Model):
    ...
    def create_booking(cls, homeowner_user_id: int, cleaner_user_id: int, service_id: int) -> tuple[dict, int]:
        ...
        if not service or not cleaner:
            return {"error": "Invalid service or cleaner"}, 404
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    query = query.filter(cls.booking_date >= start)
                except ValueError:
            ...
            if end_date:
                try:
                    ...
                    query = query.filter(cls.booking_date <= end)
                except ValueError:
